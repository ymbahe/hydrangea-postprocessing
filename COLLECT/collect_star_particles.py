...
outdir = '/virgo/scratch/ybahe/HYDRANGEA/STAR_CATALOGUES/'

extra_fields = True
individual_files = True

def extract_dataset(snapdir, outloc, dset):

    data = st.eagleread(snapdir, 'PartType4/' + dset, astro = False)
    cgsf = yb.read_hdf5_attribute(snapdir, 'PartType4/' + dset, 
                                  'CGSConversionFactor')
    vd = yb.read_hdf5_attribute(snapdir, 'PartType4/' + dset, 
                                'VarDescription')
    ase = yb.read_hdf5_attribute(snapdir, 'PartType4/' + dset, 
                                 'aexp-scale-exponent')
    hse = yb.read_hdf5_attribute(snapdir, 'PartType4/' + dset, 
                                 'h-scale-exponent')
...
def process_snapshot(isnap):
    ...
    if extra_fields:
        outloc = outdir + ('/CE-{:d}/CE-{:d}_snap_{:03d}.hdf5' 
                           .format(isim, isim, isnap))
    else:
        outloc = outdir + ('/CE-{:d}/CE-{:d}_snap_{:03d}_core.hdf5'
                           .format(isim, isim, isnap))
    ...
    if extra_fields:
        for dset in ['StellarFormationTime', 'SmoothedMetallicity',
            ...
            extract_dataset(snapdir, outloc, dset)
    else:
        for dset in ['StellarFormationTime', 'SmoothedMetallicity',
