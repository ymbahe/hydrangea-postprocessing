...
outloc = '/virgo/scratch/ybahe/TESTS/GalaxyMassEvol_CE-11_G-8371_CC28.hdf5'

igal = 8371
cenGalID = None#10589#39902
...
for isnap in range(30):
    ...
    if shi_cantor[isnap] >= 0:
        mass_cantor[isnap] = np.log10(yb.read_hdf5(cantorloc, 'Snapshot_' + str(isnap).zfill(3) + '/Subhalo/Mass')[shi_cantor[isnap]]) + 10.0
        mstar_cantor[isnap] = np.log10(yb.read_hdf5(cantorloc, 'Snapshot_' + str(isnap).zfill(3) + '/Subhalo/MassType')[shi_cantor[isnap], 4]) + 10.0
        mdm_cantor[isnap] = np.log10(yb.read_hdf5(cantorloc, 'Snapshot_' + str(isnap).zfill(3) + '/Subhalo/MassType')[shi_cantor[isnap], 1]) + 10.0
        mgas_cantor[isnap] = np.log10(yb.read_hdf5(cantorloc, 'Snapshot_' + str(isnap).zfill(3) + '/Subhalo/MassType')[shi_cantor[isnap], 0]) + 10.0
        vd_stars_cantor[isnap] = yb.read_hdf5(cantorloc, 'Snapshot_' + str(isnap).zfill(3) + '/Subhalo/VelocityDispersion_Stars')[shi_cantor[isnap]]

        vd_dm_cantor[isnap] = yb.read_hdf5(cantorloc, 'Snapshot_' + str(isnap).zfill(3) + '/Subhalo/VelocityDispersion_DM')[shi_cantor[isnap]]

        extra_id = yb.read_hdf5(cantorloc, 'Snapshot_' + str(isnap).zfill(3) + '/Subhalo/Extra/ExtraIDs')[shi_cantor[isnap]]
        if extra_id >= 0:
            kappa_co = yb.read_hdf5(cantorloc, 'Snapshot_' + str(isnap).zfill(3) + '/Subhalo/Extra/Stars/KappaCo')[extra_id, :]
