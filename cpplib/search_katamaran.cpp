...
long search_katamaran(const std::vector<unsigned long> &vTaskIDs_A,       // Source IDs [original order]
		      ...
		      std::vector<long> &vMatchInds, // [O] Match-vector (see above) 
	...
	// Match!?! Great! Update both.
	else if (val_a == val_b)
	  {
	    if (nFlagLocalIndices == 0)
	      vMatchInds.at(sortLocA) = sortLocB + PartOffset_B;
	    else
	      vMatchInds.at(sortLocA) = sortLocB;
...
// **************************************************************************************
// Main function: index_to_halo()
// Converts particle index positions (from search_katamaran()) into halo indices
// **************************************************************************************
...
std::vector<int> index_to_halo(std::vector<long> &vIndex,              // Index list (particles)
  ...
  if (mpirank() == 0)
    cout << rp() + "   Sorting matched indices took " << ElTime() << "sec. " << endl;
  ...
  while (vIndex.at(vSortVec.at(i_min)) < 0) { 
    ...
    if (i_min >= vSortVec.size()) {
      cout << rp() + "No matches at all???" << endl;
      i_min = -1;
      break;
    }
  }
  ...
  if (i_min < 0) {
    cout << rp() + "Found no matching IDs at all. Proceed at own risk..." << endl;
    return vHaloes; 
  }
  ...
  long i = i_min;
  long h = 0;
  long currhalo_max = vOffset.at(h+1)-1;
